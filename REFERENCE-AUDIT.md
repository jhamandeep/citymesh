...
### Source environment in coordinated IFC
The optional “Include terrain & buildings in BIM” export adds the USGS terrain mesh and individual Austin source buildings, and raises all 30 site placements and transport endpoints to their sampled ground elevations. It retains source IDs/years, height status, conversion units and datum caveats. The five unknown-height buildings export as flat footprint annotations, not placeholder-height solids. The original flat design-only download remains available. Independent IFC parsing verifies all 8,181 terrain vertices, 2,038 height-bearing buildings, five footprint annotations, per-site and endpoint Z coordinates, metadata and reference integrity. UI tests download both variants and check their contents; environment output is approximately 29 MB. External BIM viewer certification and independent vertical-datum alignment remain unproven.

### Component-linked inspection photo evidence
Site workspaces now retain up to 20 JPEG/PNG/WebP originals per site (5 MB each), linked to equipment ID and the entry revision, with capture date, source, notes and SHA-256 provenance. Photos reopen from IndexedDB, filter by selected component, locate surviving equipment, download originals/provenance and require confirmation before removal. File signatures and browser decoding are checked before storage. Evidence is browser-local and excluded from shared portfolio/JSON/BIM exports; source notes are not authenticated signatures. Tests cover persistence, site isolation, byte integrity, quota failure preserving evidence, concurrent count limits, actual panel association/filter/locate/download/removal flows and decoder rejection (decoder stubbed in jsdom). Actual browser photo rendering remains unverified.

### Shared photo evidence across devices
Added private R2 original-file storage and D1 metadata for explicitly shared inspection photos. Each shared record references an immutable saved portfolio version containing its equipment ID and matching project revision. This association does not establish surveyed geometry or authenticated capture identity. The UI supports sharing a local photo, reopening shared originals without local copies, equipment location, provenance downloads, refresh and confirmed shared deletion. Same-origin mutations, bounded uploads, signature/checksum validation, 20-photo site limits and idempotent retries are enforced. Tests exercise real local workerd/D1/R2 requests and actual React controls, including revision mismatch, duplicate attempts, metadata conflicts, byte-exact downloads, cross-site isolation and removal. An ambiguous D1 failure after object upload may leave an unreferenced R2 object; deleted metadata is authoritative and object cleanup errors are logged. Production access remains owner-private. Survey GLB sharing, authenticated evidence signatures and live telemetry remain outstanding.
