...
render(React.createElement(Sites));fireEvent.click(screen.getByRole('button',{name:'Flat map'}));assert.ok(screen.getByRole('heading',{name:'One network. Every site connected.'}));assert.equal(screen.getAllByRole('row').length,31);assert.ok(document.querySelectorAll('.circuit-trace li').length>5);fireEvent.click(document.querySelector('.cable-list button'));assert.ok(document.querySelector('.cable-detail').textContent.includes('LC/UPC'));
