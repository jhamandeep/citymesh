...
render(React.createElement(Sites));assert.ok(screen.getByRole('heading',{name:'One network. Every site connected.'}));assert.equal(screen.getAllByRole('row').length,31);
const physicalLinks=screen.getAllByRole('link').map(a=>a.getAttribute('href')).filter(h=>h?.startsWith('/sites?site='));assert.equal(new Set(physicalLinks).size,30);
...
fireEvent.click(screen.getByRole('button',{name:'Simulate complete site outage'}));assert.equal(document.querySelector('.network-detail .site-health').textContent,'Disconnected');
fireEvent.click(screen.getByRole('button',{name:'Restore simulated site'}));assert.notEqual(document.querySelector('.network-detail .site-health').textContent,'Disconnected');
fireEvent.click(screen.getByRole('button',{name:/^L01:/}));fireEvent.click(screen.getByRole('button',{name:'Simulate link failure'}));assert.ok(screen.getByText('Offline',{exact:true,selector:'dd'}));fireEvent.click(screen.getByRole('button',{name:'Restore transport link'}));assert.ok(screen.getByText('Available',{exact:true,selector:'dd'}));
