...
fireEvent.click(screen.getByRole('button',{name:'Select GBT-03 geographic site'}));assert.ok(screen.getByRole('heading',{name:'GBT-03 · RF parameters'}));const oldRadius=document.querySelector('.rf-kpis').textContent;fireEvent.change(screen.getByRole('spinbutton',{name:'TX power (dBm)'}),{target:{value:'40'}});assert.notEqual(document.querySelector('.rf-kpis').textContent,oldRadius);assert.equal(JSON.parse(localStorage.getItem('citymesh-rf-v1'))['GBT-03'].power,40);fireEvent.click(screen.getByRole('checkbox',{name:'coverage',exact:true}));assert.equal(document.querySelectorAll('.geo-map polygon').length,0);fireEvent.click(screen.getByRole('checkbox',{name:'coverage',exact:true}));assert.equal(document.querySelectorAll('.geo-map polygon').length,84);fireEvent.click(screen.getByRole('button',{name:'Reset this site RF'}));assert.equal(screen.getByRole('spinbutton',{name:'TX power (dBm)'}).value,'30');
