...
const portfolio=createPortfolio(),model=createNetworkModel(portfolio);assert.equal(model.lods.size,30);assert.equal(model.assets.size,254);assert.equal(model.cables.size,610);assert.equal(model.links.size,38);const camera=new THREE.PerspectiveCamera();
...
console.log('Network geometry verified: 30 detailed sites, 254 positioned assets, 610 local cable meshes, 38 exact termination-to-termination paths, near/far detail switching and world-space asset picking. WebGL pixels and frame rate are not measured.');
