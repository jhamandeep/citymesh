...
export const defaultSurveyAlignment = (): SurveyAlignment => ({
  offset: [0, 0, 0],
  yaw: 0,
  scale: 1,
  overlay: false,
...
export function validateAlignment(value: unknown): SurveyAlignment {
  ...
  if (!value || typeof value !== 'object')
    throw new Error('Invalid survey alignment.');
  ...
  if (
    ...
    a.scale < 0.001 ||
    a.scale > 1000 ||
    typeof a.overlay !== 'boolean'
  )
    throw new Error(
      'Use offsets within ±10,000 m, rotation within ±360°, and scale from 0.001 to 1,000.',
    );
  return {
    offset: [...a.offset],
    yaw: a.yaw,
...
export function applySurveyAlignment(
  group: THREE.Group,
  value: SurveyAlignment,
) {
  const a = validateAlignment(value);
  group.position.set(...a.offset);
  group.rotation.set(0, (-a.yaw * Math.PI) / 180, 0);
...
>();
export function setSurveyOverlay(group: THREE.Group, overlay: boolean) {
  group.traverse((object) => {
    ...
    for (const material of Array.isArray(object.material)
      ...
      material.opacity = overlay ? original.opacity * 0.3 : original.opacity;
      material.transparent = overlay || original.transparent;
      material.depthWrite = overlay ? false : original.depthWrite;
