...
export async function photoDigest(bytes:ArrayBuffer){return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes)),v=>v.toString(16).padStart(2,'0')).join('');}
...
export async function makePhoto(input:Pick<PhotoRecord,'siteId'|'assetId'|'revision'|'filename'|'bytes'|'source'|'capturedAt'|'note'>):Promise<PhotoRecord>{return validatePhoto({...input,id:crypto.randomUUID(),mime:photoMime(input.bytes),sha256:await photoDigest(input.bytes),addedAt:new Date().toISOString()});}
...
export async function listPhotos(siteId:string){
 ...
 for(const r of records){validatePhoto(r);if(await photoDigest(r.bytes)!==r.sha256)throw new Error('Saved photo failed its integrity check.');}
 return records.sort((a,b)=>b.addedAt.localeCompare(a.addedAt));
}
export async function savePhoto(r:PhotoRecord){
 validatePhoto(r);if(await photoDigest(r.bytes)!==r.sha256)throw new Error('Photo checksum does not match its file.');const db=await open();
