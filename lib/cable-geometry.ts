...
export function createCableCurve(c:PhysicalCable){
 ...
 const points=a.distanceTo(b)<.05?[a,a.clone().add(new THREE.Vector3(.25,.12,.2)),b]:[a,a.clone().add(new THREE.Vector3(0,0,.3+lane)),new THREE.Vector3(b.x,a.y,b.z+.3+lane),b.clone().add(new THREE.Vector3(0,0,.3+lane)),b];
 return new THREE.CatmullRomCurve3(points,false,'centripetal');
}
export function createCableMesh(c:PhysicalCable,selected=false){
 const a=new THREE.Vector3(...c.from.position),b=new THREE.Vector3(...c.to.position),curve=createCableCurve(c);
