...
cleanup();for(const [id,type,name] of [['GBT-08','GBT','Transit corridor'],['RTT-03','RTT','Security command rooftop'],['IBS-10','IBS','Logistics terminal'],['SC-02','SMALL_CELL','Staff transit hub'],['CORE-02','CORE','Disaster recovery core']]){window.history.replaceState({},'','/sites?site='+id);render(React.createElement(Sites));assert.ok(screen.getByRole('heading',{name}));assert.equal(screen.getByTestId('site-model-type').textContent,type);assert.ok(screen.getByTestId('selected-asset').textContent);cleanup();}window.history.replaceState({},"","/sites?site=GBT-08&asset=ANT-B");render(React.createElement(Sites));assert.equal(screen.getByTestId("selected-asset").textContent,"ANT-B");cleanup();dom.window.close();console.log('Site control integration passed: model selection adapter, invalid design rejection, editing, approval, evidence-gated acceptance, issue lifecycle, revision restart, and persistence after remount. WebGL rendering is not mocked into a claimed visual pass.');
